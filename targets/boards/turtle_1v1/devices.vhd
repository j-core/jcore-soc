...
architecture impl of devices is
    ...
    signal sig_rtc_nsec : std_logic_vector(31 downto 0);
    signal pps     : std_logic;
    signal pps_sec : std_logic;
...
begin
    ...
    word_ack_gen : entity work.word_ack_gen ( impl )
      port map (
        ...
        ack_thru_in_gpsif    => devs_bus_i(DEV_GPSIF).ack,
    ...
    aic0 : entity work.aic(behav)
        generic map (
            ...
            vector_numbers => (x"11", x"12", x"13", x"14", x"15", x"00", x"00", x"00")
        )
        port map (
    ...
    pps0: process(clk_sys, reset, sig_rtc_nsec)
	  begin
             if reset = '1' then pps <= '0'; pps_sec <= '0';
	     elsif clk_sys'event and clk_sys = '1' then
		if pps_sec /= sig_rtc_nsec(29) and sig_rtc_nsec(29) = '1' then pps <= not pps; end if;
		pps_sec <= sig_rtc_nsec(29);
	     end if;
          end process;
    gpsif : entity work.gpsif_top(arch)
        generic map (
            angle_ini_old => FALSE,
            gpsif_nc => 3
            -- gpsif_nc => 5
        ...
        port map (
            ...
            clk => clk_sys,
            -- clk => clk_gpsif,
            db_i => devs_bus_o(DEV_GPSIF),
            db_o => devs_bus_i(DEV_GPSIF),
            dma => open,
            gps_clk => gps_clk,
            gps_d => gps_d,
            intrpt => irqs0(2),
