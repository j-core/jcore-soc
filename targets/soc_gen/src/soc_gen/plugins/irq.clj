...
(defn- update-device-tree
  "Updates the device tree based for the interrupt controller(s) and interrupts"
  [design aic-devs pit-irq]
  (if (empty? aic-devs)
    design
    (let [;; The device tree represents the aic and pit as a single node
          ;; with a multiple reg ranges.
          addrs (->> aic-devs
                     (map :base-addr)
                     sort)

          aic-class (get-in design [:device-classes (:class (first aic-devs))])
          reg-width (bit-shift-left 1 (inc (:left-addr-bit aic-class)))

          timer-props [[:compatible "jcore,pit"]
                       ;; TODO: avoid hardcoding 0x30 register region width
                       [:reg {:fmt :hex :v (apply list (interleave addrs (repeat 0x30)))}]
                       [:interrupts {:fmt :hex :v (list pit-irq)}]]

          aic-reg {:fmt :hex :v (apply list (interleave addrs (repeat reg-width)))
                   :cmt (s/join ", "
                                (map
                                 #(format "%08X-%08X" % (+ % reg-width -1))
                                 addrs))}
          aic-in-dt (some (fn [aic] (when (not= (:dt-node aic) false)
                                      (:name aic)))
                          aic-devs)]

      (-> design
          ;; Add a timer node to device tree at same address offset as aic node.
          (update-in [:soc :dt-children]
                     (fn [dt-children]
                       (conj dt-children ["timer" {:properties timer-props}])))
          ;; Set reg of aic
          (update-in [:devices]
                     (fn [devs]
                       (if aic-in-dt
                         (assoc-in devs [aic-in-dt :dt-props :reg] aic-reg)
                         devs)))))))
...
(defrecord AIC1Plugin []
    ...
    (on-pregen [plugin design]
      (let [;; Auto set irq and path for devices with only irq
            ...
            ;; With aic1, the ipi memory-mapped registers and irq are
            ;; in the cache device
            ipi-info
            (let [cache-dev
                  (->> (vals (:devices design))
                       (filter #(= "cache_ctrl" (:class %)))
                       first)]
              ...
              (when cache-dev
                {:irqs
                 (->> (:irq cache-dev)
                      ...
                      set)
                 :addr (:base-addr cache-dev)}))]

        (when-not pit-trap
          (errors/add-error "Cannot determine unused irq for pit"))
        ...
        [(assoc plugin
                ...
                (->> aic-devs
                     ...
                     (into {})))
         (-> design
             (update-device-tree aic-devs pit-trap)
             (assoc :ipi-info ipi-info))]))
    (file-list [plugin])
...
(defrecord AIC2Plugin []
    ...
    (on-pregen [plugin design]
      (let [;; find aic devices to find their aic ports
            ...
            ;; select unused trap number for pit. Avoid 64-71 which yield 0-priority
            pit-trap
            (some #(when-not (contains? used-traps %) %) (range 72 128))

            ;; With aic2, the ipi memory-mapped registers are in the
            ;; cache device but the ipi irq the first generic of the aic
            ipi-info
            (let [cache-dev
                  (->> (vals (:devices design))
                       (filter #(= "cache_ctrl_wsbu" (:class %)))
                       first)

                  aic2-class
                  ((:device-classes design) "aic2")

                  ;; lookup both the default generic value
                  default-ipi
                  (let [def (get-in aic2-class [:entity :generics "irq_si0_num" :default-value])]
                    (when (instance? DecimalLiteral def)
                      (Long/parseLong (.getValue def))))
                  ;; and also lookup the generic values set for each instantiation of aic2
                  ipi-irqs
                  (->> (vals aic-devs)
                       (map
                        #(or (get-in % [:generics "irq_si0_num"])
                             default-ipi))
                       (filter identity)
                       set)]
              ...
              (cond-> {:irqs ipi-irqs}
                cache-dev (assoc :addr (:base-addr cache-dev))))]
        ...
        [(assoc plugin
                ...
                :null-irq null-irq
                :irq-a-t irq-a-t)
         (-> (update-device-tree design (vals aic-devs) pit-trap)
